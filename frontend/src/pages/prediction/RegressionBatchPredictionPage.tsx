...
export default function RegressionBatchPredictionPage() {
  ...
  const uploadProps: UploadProps = {
    ...
    beforeUpload: (file) => {
      if (!modelId) { message.warning("请先选择回归模型"); return false; }
      setPendingFile(file); setResult(null);
  ...
  async function handleStartPredict() {
    ...
    setPredicting(true);
    try {
      setResult(await runBatchRegressionPrediction(modelId, pendingFile));
      message.success("批量回归预测完成");
    } catch { message.error("批量预测失败"); }
    finally { setPredicting(false); }
  }

  const columns: ColumnsType<Record<string, unknown>> = useMemo(() => {
    const first = result?.rows[0]; if (!first) return [];
  ...
  const rows = useMemo(() => (result?.rows ?? []).map((row, index) => ({ ...row, __row_id: index })), [result?.rows]);
  ...
  return (
    <main>
      ...
      <section className="prediction-form">
        ...
        {predicting && <div className="prediction-batch-spin"><Spin tip="正在预测中..." size="large" /></div>}
      </section>
